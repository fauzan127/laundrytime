...
<div class="bg-white p-6 rounded-lg shadow-md w-full max-w-full">
    ...
    {{-- Container ini memiliki overflow-x-auto untuk mengaktifkan scroll horizontal hanya pada area tabel. --}}
    <div class="overflow-x-auto shadow-sm border border-gray-200 rounded-lg">
        {{-- Menggunakan w-full pada layar kecil dan min-w-max pada layar besar untuk memastikan tabel mengisi ruang tapi tetap bisa digeser jika lebar totalnya melebihi container. --}}
                ...
                @forelse ($orders as $order)
                    <tr class="hover:bg-gray-50 transition duration-150">
                        ...
                        @if(Auth::check() && Auth::user()->role === 'admin')
                            ...
                            {{-- Ubah max-w-xs menjadi max-w-sm dan tambahkan style/class untuk word-break agar teks tidak melebar terlalu jauh --}}
                            <td class="px-6 py-4 text-sm text-gray-700 max-w-sm overflow-hidden text-clip" style="word-break: break-word;">{{ $order->address ?? 'N/A' }}</td>
